...
# ------------------------------------------------------------
# Color Palettes and Custom Colormaps
# ------------------------------------------------------------
...
def add_geodesic_scale_bar(ax, pad=0.02, height_frac=0.02, label_pad=0.006, offset=0.08, fontsize=10):
    geod = Geod(ellps="GRS80")  # EPSG:4269 (NAD83) ellipsoid
    ...
    # True geodesic width (km) across the current extent at mid-lat
    ...
    target = width_km / 5.0
    nice = np.array([1, 2, 5, 10, 20, 50, 100, 200, 500, 1000])
    length_km = float(nice[np.argmin(np.abs(nice - target))])

    # Convert that km length to degrees of longitude at lat_mid
    _, _, m_per_deg_lon = geod.inv(lon_mid, lat_mid, lon_mid + 1.0, lat_mid)
    deg_lon_for_length = (length_km * 1000.0) / m_per_deg_lon

    # Geometry for the bar: anchored at upper-right, then shifted LEFT by 'offset'
    xspan = xmax - xmin
    yspan = ymax - ymin
    xpad = pad * xspan
    ypad = pad * yspan
    bar_h = height_frac * yspan

    # Right edge of bar is offset from the upper-right corner
    x1 = xmax - xpad - offset * xspan
    x0 = x1 - deg_lon_for_length
    y0 = ymax - ypad - bar_h
    label_y = y0 + bar_h + label_pad * yspan

    # Draw bar + label
    ax.add_patch(plt.Rectangle((x0, y0), x1 - x0, bar_h, facecolor="k", edgecolor="k", zorder=10))
    ax.text((x0 + x1) / 2, label_y, f"{int(length_km)} km", ha="center", va="bottom",
...
class VisData:
    ...
    # ------------------------------------------------------------
    # Load and preprocess adoption/benchmark data
    # ------------------------------------------------------------
    def loadData(self):
        ...
        for county in self.counties:
            ...
            for j in range(batch_num):
                try:
                    ...
                    FOLDER_PATH = 'fit_pq_'+''.join(i.lower() for i in county.split())+'_'+str(j)+'_'+str(number_node)+'_'+str(-1)+'_'+str(10)+'_'+str(3)+'_'+str(3)
                    for i in range(10):
                        sub_tract = (pd.read_csv(os.path.join(self.RESULT_PATH, county, FOLDER_PATH, 'curves', 'tract_curve_'+str(i)+'.csv'),index_col=0).cumsum(axis=0).transpose()/scale)
                        low = emp_pop_group[0].merge(sub_tract,left_on='GEOID',right_on=sub_tract.index)[['GEOID']+list(range(end))]
                        low['REP'] = i; low['BATCH'] = j; low_curve.append(low)
        ...
        for county in self.counties:
            ...
            for j in range(batch_num):
                try: 
                    folder = 'fit_pq_'+''.join(i.lower() for i in county.split())+'_'+str(j)+'_'+str(number_node)+'_'+str(-1)+'_'+str(10)+'_'+str(3)+'_'+str(3)
    ...
    # ------------------------------------------------------------
    # 1. Current adoption visualization
    # ------------------------------------------------------------
    def currentAdoption(self):
        ...
        ax = data_map.plot(ax = axs, column="RATE", cmap=cm, edgecolor=None, legend=False,linewidth=0.5)
        if self.state == 'ca':
            ax.set_xlim(-130, -110)
            ax.set_ylim(31, 43)
        else:
        ...
        cx.add_basemap(ax, source=cx.providers.CartoDB.PositronNoLabels, crs=data_map.crs,attribution_size=3)
        
        north_arrow(
            ax,
            location="upper left",
            rotation={"crs": data_map.crs, "reference": "center"},
            label = {"fontsize": 10, "position": "right"},
            scale=0.15,
            shadow=False
        )
        add_geodesic_scale_bar(ax, pad=0.15, height_frac=0.005, label_pad=0.006, offset=-0.08, fontsize=10)
    ...
    # ------------------------------------------------------------
    # 3. Describe adoption (error distributions, temporal curves)
    # ------------------------------------------------------------
    def describeAdoption(self):
        if self.state=='wa':
            colorb = 'blue'; colorp = 'blue'
            colors = ['#81dbed','#60a3f2','blue'] 
            cm = ['#1F51FF', '#1F51FF', '#1F51FF']
            county = 'King'
            al = 1
        else:
        ...
        hatch_color = 'k'
        gfg = sns.histplot(data = data_error, x="Error", hue="Model", stat="percent", bins = 100, multiple="dodge", shrink=.8, edgecolor=hatch_color,linewidth=1, palette=cm,alpha=al)
        hatches = ['x','*','o']
        for container, hatch, handle in zip(gfg.containers, hatches, gfg.get_legend().legend_handles[::-1]):
            handle.set_hatch(hatch)
            for rectangle in container:
                rectangle.set_edgecolor(hatch_color)
        ...
        axs = fig.add_subplot()
        sns.scatterplot(ax = axs, data=data_error, x='Data', y='SocNet',c='#81dbed',s=3, label = 'SocNet',rasterized=True)
        sns.scatterplot(ax = axs, data=data_error, x='Data', y='County',c='blue',s=3, label = 'County',rasterized=True)
        sns.scatterplot(ax = axs, data=data_error, x='Data', y='State',c='orange',s=3, label = 'State',rasterized=True)
        axs.axline((1, 1), slope=1, c = 'k', linestyle='dashed')

        plt.xlim(0, 10)
        plt.ylim(0, 10)

        lgnd = plt.legend(markerscale=5, loc='upper left')
        plt.xlabel('Adoption Rate of Data [%]')
        plt.ylabel('Adoption Rate of Model [%]')
        plt.savefig(os.path.join(self.FIGURE_PATH, 'si_scatter_error.pdf'),dpi=300,bbox_inches='tight')
        ...
        plt.plot(np.arange(2010,2023,2),np.percentile(dtl[de_names].values,mid_b,axis=0)[::2]/pop_low, color = colors[0], alpha= 0.7,marker='*', markersize=10 )
        plt.plot(np.arange(2010,2023,2),np.percentile(dtl[ds_names].values,mid_b,axis=0)[::2]/pop_low, color = colors[0], alpha= 0.7, marker='x',markersize=10)
        plt.scatter(np.arange(2010,2023),np.percentile(dtl[emp_names].values,mid_b,axis=0)/pop_low,  color = colors[0], alpha= 1, marker='s' )
        ...
        plt.plot(np.arange(2010,2023,2),np.percentile(dtl[de_names].values,mid_b,axis=0)[::2]/pop_mid, color = colors[1], alpha= 0.7,marker='*', markersize=10)
        plt.plot(np.arange(2010,2023,2),np.percentile(dtl[ds_names].values,mid_b,axis=0)[::2]/pop_mid, color = colors[1], alpha= 0.7,marker='x',markersize=10)
        plt.scatter(np.arange(2010,2023),np.percentile(dtl[emp_names].values,mid_b,axis=0)/pop_mid,  color = colors[1], alpha= 1, marker='s' )

        dtl = data_temp_high
        p_5 = np.percentile(dtl[abm_names].values,low_b,axis=0)                
        p_50 = np.percentile(dtl[abm_names].values,mid_b,axis=0)
        p_95 = np.percentile(dtl[abm_names].values,high_b,axis=0)
        plt.plot(np.arange(2010,2023,2),np.percentile(dtl[ds_names].values,mid_b,axis=0)[::2]/pop_high, color = colors[2], alpha= 0.7, marker='x',label = 'State',markersize=10)
        plt.plot(np.arange(2010,2023,2),np.percentile(dtl[de_names].values,mid_b,axis=0)[::2]/pop_high, color = colors[2], alpha= 0.7, marker='*', label = 'County',markersize=10)
        plt.plot(np.arange(2010,2023,2),p_50[::2]/pop_high, color = colors[2], alpha= 0.5, marker='o', label = 'SocNet')
        plt.scatter(np.arange(2010,2023,2),np.percentile(dtl[emp_names].values,mid_b,axis=0)[::2]/pop_high,  color = colors[2], alpha= 1,  marker='s',label = 'Data')
